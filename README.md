...
1. 구현할 기능 상세 목록 작성
    - 상세 목록이 구체화 되지 않을 경우 기능을 구현하면서 문서을 작성
2. 기능 구현
    - 구현하는 과정에서 추가되는 기능이 발생할 경우, 기능 목록을 최신화
3. 테스트할 케이스 목록 작성
4. 테스트 코드 작성
    - 테스트를 통과하지 못하거나, 테스트 작성 과정에서 누락-개선해야할 기능 발견시 1,2번 항목으로 다시 진행

> 각 항목이 완료 될 때마다 commit 진행
...
> 구매 금액에 대한 유효성 검사
>
> 1. 에러에 해당하는 메세지를 담은 정보를 선언
> 2. 에러 메시지 정보로 필드 초기화
> 3. 매개 변수로 에러 상태를 저장할 필드를 최신화
> 4. 최신화된 상태를 확인
...
> 입력값으로 자료구조를 생성
>
> 1. LottoList.js: 구매 금액으로 구매 목록 생성
> 2. LottoResult.js: 구매 목록, 당첨 번호, 보너스 번호로 당첨 기록 생성
> 3. ResultString.js: 당첨 기록으로 문자열 생성
...
> 구매목록, 당첨 번호, 보너스 번호로 당첨 기록 생성
>
> 1. 당첨 기록 초기화
> 2. 반복문의 비동기 실행 결과 반환
> 3. 당첨 목록 생성
> 4. 기록할 필요가 없는 당첨 목록 필터링
> 5. 당첨 목록으로 당첨 기록 최신화
> 6. 당첨 수익률 계산
...
##### Case: checkBonusNumber 유효한 케이스

- 당첨 번호 내의 요소와 보너스 번호가 중복되지 않은 경우
- 보너스 번호가 1 ~ 45 사이의 값인 경우

#### Case: checkBonusNumber 유효하지 않은 케이스
